...
The `INLA` has its own object and conventions. The `eflINLA` is a project to develop functions mimicking `tidybayes` with `inla` objects. These are **not S3 methods** and they will be identified with the suffix `inla`, for example the `tidy_draws` equivalent is `tidy_draws_inla`.
...
We use the `simstudy` package to do the simulation. It is a wonderful tools to 
simulate models and will be used repeatedly in this project.

Also we use the `posterior` package to use the many tools this package offers 
to simplify using posterior samples.

```{r}
sim04M01 <- list()
sim04M01 <- within(sim4M1, {
...
ggplot(sim04M01$data, mapping = aes(x = height)) +
  ggdist::stat_halfeye(aes(fill = stat(cut_cdf_qi(cdf, .width = sim04M01$widths))),
...
m04M07ctr <- xfun::cache_rds({
  quap(
    ...
    data = data04M07,
    start = list(a = mean(data04M07$weight), b = 0.5)
    )},
  file = "ch04_m04_07ctr")
tictoc::toc()
```
...
postQuap <- list()
postQuap <- within(postQuap, {
  nsamples <- 500L
  ctr <- list()
  ctr$post <- tidy_draws(m04M07ctr, n = nsamples) |>
    ...
    identity()
  nat <- list()
  nat$post <- tidy_draws(m04M07nat, n = nsamples) |>
    select(-.chain, -.iteration) |>
    # mutate(model = 2) |>
    as_draws_df() |>
  ...
  all <- list()
  all$post <- bind_rows("ctr" = ctr$post, "nat" = nat$post, .id ="model")
...
```{r}
ggplot(postQuap$all$lng, aes(x = .value, color = model)) +
  ...
  labs(title = "4M7: Posterior comparisons by model using quap",
       subtitle = sprintf("sample size = %d", postQuap$nsamples),
       x = NULL, y = NULL)
```
...
postQuap <- within(postQuap, {
  ...
  ctr$predict <- predicted_draws(
    m04M07ctr,
    newdata = ctr$newdata,
    ndraws = nsamples)
  # get the intervals
  ctr$predict_intr <- median_qi(ctr$predict) %>%
  ...
  nat$predict <- predicted_draws(
    m04M07nat, 
    newdata = nat$newdata, 
    ndraws = nsamples)
  # get the intervals
  nat$predict_intr <- median_qi(nat$predict)
  
  # put all intervals together
  all$predict_intr <- bind_rows(
    "ctr"= ctr$predict_intr, "nat" = nat$predict_intr, .id = "model")
  })
```
...
```{r}
ggplot(postQuap$all$predict_intr, 
...
```{r}
postBrm <- list()
postBrm <- within(postBrm, {
  nsamples <- 500L
  ctr <- list()
  ctr$post <- tidy_draws(b04M07ctr, n = nsamples) |>
    # .chain and .iteration are NA and will generate error
    select(-.chain, -.iteration) |>
    # mutate(model = 1) |>
    as_draws_df() |>
    ...
    identity()
  nat <- list()
  nat$post <- tidy_draws(b04M07nat, n = nsamples) |>
    select(-.chain, -.iteration) |>
    # mutate(model = 2) |>
    as_draws_df() |>
    # mutate_variables(model = 2) |>
...
```{r}
ggplot(postBrm$all$predict_intr, 
       aes(x = weight, y = .prediction, ymin = .lower, ymax = .upper,
           color = model)) +
  geom_pointinterval(position = position_dodge(width = 0.5),
                     fatten_point = 2, size = 1) +
...
plot_04M08 <- function(data, x_var = "year", y_var = "doy", color_var = "temp", 
  ...
  titles <- list(
    title = sprintf("4M8: Cherry Blossom in Japan with %d knots", length(knots)),
...
splinA <- within(splinA,{
  ...
  fitted <- linpred_draws(b04M08a, newdata = newdata) %>%
    as.data.frame() %>%
    select(-B) |>
    group_by(year) |>
    mean_qi() |>
...
p$A <- plot_04M08(data04M08, knots = splinA$knots) +
  labs(subtitle = "4M8 a)") +
  geom_smooth(data = splinA$fitted, 
...
See section 9.2, p. 204, of @gomez2020 on using `inla` with *B-spline*. Also, see section 3.2, p.47 of @wang2018 on setting the priors.
...
The coefficients from `inla` include extra coefficients at the beginning and end which are near zero with very large sd.
...
We observe that `inla` and `brm` generate coefficients and their errors which are similar.
...
and the prediction intervals are obtained as described in section 4.4.3.5 using `rethinking::sim()` (sames as in 4H2 above)
...
sample the prior, simulate the prior predictive distribution and calculate the intervals
...
Conclusion: The prior affect the weights the weights in the same manner as the priors affects regression coefficients. In the current example, by increasing the sd of the normally distributed prior we affect the variability of the weights.
...
The model is changed to remove the intercept which is **mu \<- B %*% w instead of*** *mu \<- a + B %*% w
