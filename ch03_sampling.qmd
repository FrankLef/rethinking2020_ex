...
::: {.callout-warning}
Published solution on internet has 5 instead of 4!
...
and we compute the posterior probability of every $p_water$ in the grid, 
given $n_tosses$. Since we don't have $y$, that is the observed nb of successes, 
we simulate them, assuming $p=0.7$ with
`rbinom(x = 1, size = the_grid$n_tosses, prob = 0.7)`.
...
> The answer is therefore that the nb of tosses should >= 2000, assuming a flat 
uniform prior, using increments of 500 for the nb 
of tosses.
...
Get a sample of $p$ from the grid using the posterior as probability of the 
given value. Then make a predicition of boys using these sampled $p$ values
